...
.item .item_mypage_name, .item .item_mypage_phone, .item .item_mypage_pw, .item .item_mypage_quit {
	display: none;
	padding-top: 10px;
	padding-bottom: 10px;
}
	...
	<section class="item">
		<div class="item_header">
			...
			<div class="item_body item_mypage">
				...
				<button id="btn_pic_view" class="btn_pic" onclick="showpicarea();">
					<div id="pic" class="mypage_header">
						<span class="mypage_frist_span"><b>프로필 :</b></span>
						<span><img class="snb_profile" src="<%=request.getContextPath()%>/upload/profile/<%=loginEmp.getProfilePic() %>" alt="프로필 사진" name="infoUserPic"></span>
				...
				<button id="btn_name_view" class="btn_name" onclick="shownamearea();">
					<div class="mypage_header">
				...
				<button id="btn_phone_view" class="btn_phone" onclick="showphonearea();">
					<div class="mypage_header">
				...
				<button id="btn_password_view" class="btn_password" onclick="showpasswordarea();">
					<div class="mypage_header">
						<span><b>비밀번호 :</b></span>
						<span>********</span>
				...
				<button id="btn_quit_view" class="btn_quit" onclick="showquitarea();">
					<div class="mypage_header">
						<span><b>회원탈퇴 :</b></span>
...
<script>
	...
	$(function(){
		$("#btn_ori_pic").click(function(){
			$.ajax({
				url: "<%=request.getContextPath()%>/ajaxemporiproFile.do",
				data: {userId:userInfo.userId},
				success:function(data){
					$('.snb_profile').attr("src", contextPath + "upload/profile/emp_default.png");
				}
			});
		});
	});
	...
	$(function(){
		$("#profile").change(function(){
			...
			$.ajax({
				url:"<%=request.getContextPath()%>/ajaxempproFile.do",
				data:fd,
				type:"post",
				processData:false,
				contentType:false,
	...
	$(function(){
		$("#new_phone").keydown(function(key){
			if(key.keyCode == 13 ) {
				$(function(){
								...
								$.ajax({
									...
									success:function(data){
										if(!data){
											...
											location.href="<%=request.getContextPath()%>/views/emp/pwdck.jsp";
											$("#pw").val('');
											$("#nPw").val('');
											$("#nkPw").val('');
										}
		...
		<form name="updateUserFrm" method="POST">
			<div class="item_body item_mypage">
				...
				<!-- <div class="item_body item_mypage_email">
					...
					<button id="btn_ck_email" class="btn-primary" onclick="return false;">인증번호 전송</button>
					<button id="btn_ch_email" class="btn-primary" onclick="return false;">이메일 변경</button>
				</div> -->
				...
				$.ajax({
					...
					success:function(data){
						if(!data){
							...
							area.style.display = "none";
							$('#btn_name_view').find('div>span').eq(1).html($('#name').val().trim());
							$('.snb_emp').find('nav>span').html($('#name').val().trim());
						}
					}
				});
			}
